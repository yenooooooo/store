...
export function build(d: TemplateData): string {
  ...
  <div style="padding:72px 40px;text-align:center;background:linear-gradient(180deg,#111 0%,#0a0a0a 100%);border-bottom:1px solid #1a1a1a;">
    ${d.trustBadges.length > 0 ? `<div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap;margin-bottom:28px;">${d.trustBadges.map((b) => `<span style="font-size:10px;color:#c4a35a;border:1px solid #2a2510;padding:4px 14px;border-radius:2px;letter-spacing:1px;text-transform:uppercase;">${b}</span>`).join("")}</div>` : ""}
    <h1 style="font-size:30px;font-weight:700;margin:0 0 16px;color:#fff;line-height:1.3;letter-spacing:-0.5px;">${d.productName}</h1>
    <p style="font-size:15px;color:#c4a35a;margin:0;font-weight:400;">${d.subtitle}</p>
    <div style="width:60px;height:2px;background:linear-gradient(90deg,#c4a35a,#8b7635);margin:28px auto 0;"></div>
  </div>
  ...
  <div style="padding:56px 40px;text-align:center;background:linear-gradient(180deg,#0a0a0a 0%,#111 100%);">
    ...
    <div style="display:inline-block;background:linear-gradient(135deg,#c4a35a,#d4b86a);color:#0a0a0a;padding:14px 48px;border-radius:4px;font-size:15px;font-weight:800;letter-spacing:1px;">SHOP NOW</div>
