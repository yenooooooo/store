...
import { hasFeatureAccess, getPlanDisplayName, isAdmin } from "@/lib/plan-features";
...
export default function Sidebar() {
  ...
  return (
    <aside className="hidden w-64 shrink-0 border-r border-white/[0.06] bg-[#0A0A0B] text-white md:flex md:flex-col">
      {/* 로고 */}
      ...
      {/* 하단 크레딧/업그레이드 배너 */}
      {profile && (
        <div className="m-3 rounded-xl bg-white/[0.04] border border-white/[0.06] p-4">
          <div className="text-xs font-medium text-white/80 mb-1">
            {getPlanDisplayName(userPlan)} 플랜
          </div>
          <div className="text-[10px] text-white/40 mb-2">
            남은 크레딧: {Math.max(0, profile.credits_limit - profile.credits_used)} / {profile.credits_limit}회
          </div>
          <div className="w-full h-1.5 rounded-full bg-white/10 mb-3">
            <div
              className="h-full rounded-full bg-white/60 transition-all"
              style={{ width: `${Math.max(0, 100 - (profile.credits_used / profile.credits_limit) * 100)}%` }}
            />
          </div>
          {profile.plan === "free" && (
            <Link href="/billing">
              <button className="w-full rounded-lg bg-white/10 hover:bg-white/15 text-white/70 text-xs font-medium py-1.5 transition-colors">
                업그레이드
              </button>
            </Link>
          )}
